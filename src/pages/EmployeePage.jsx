...
export default function EmployeePage({ employeeId }) {
  ...
  const complete = async rec => {
    ...
    try {
      await data.completeActivity(employeeId, rec.event_id);
      const [freshEmployee, freshRecs] = await Promise.all([data.getEmployee(employeeId), data.getRecommendations(employeeId)]);
      state.setEmployee(freshEmployee);
      state.setRecommendations(normalizeList(freshRecs?.recommendations ?? freshRecs));
      setChanged((rec.closes_skills || []).filter(skill => (freshEmployee.skills?.[skill] || 0) !== (before[skill] || 0)));
    } catch (err) { state.setRecError(`${err.message} Your profile and other recommendations are still available.`); }
  ...
  const employee = state.employee; const skills = Object.entries(employee.skills || {}); const criticalSet = new Set(state.recommendations.filter(r => r.critical).flatMap(r => r.closes_skills || []));
  skills.sort(([a], [b]) => Number(criticalSet.has(b)) - Number(criticalSet.has(a)) || (employee.skills[b] || 0) - (employee.skills[a] || 0));
  ...
  return <div className="page-wrap">
    ...
    <div className="employee-grid">
      <section className="panel skills-panel"><div className="section-heading"><div><div className="eyebrow">YOUR FOUNDATION</div><h2>Skills snapshot</h2></div><span className="small-count">{skills.length} SKILLS</span></div><p className="muted">Your current capability, mapped against your growth path.</p>
        <div className="skill-list">{skills.map(([id, level]) => { const critical = criticalSet.has(id); const filled = Math.max(0, Math.min(5, Number(level) || 0)); return <div className={`skill-row ${changed.includes(id) ? 'skill-changed' : ''}`} key={id}><div className="skill-label"><span>{getSkillName(id)}{critical && <span className="critical-tag">Priority skill</span>}</span><b>{filled}<small> / 5</small></b></div><div className="progress-track"><i className={critical ? 'critical-fill' : ''} style={{ width: `${filled * 20}%` }}/></div></div>; })}{!skills.length && <p className="muted">No skill data available yet.</p>}</div>
